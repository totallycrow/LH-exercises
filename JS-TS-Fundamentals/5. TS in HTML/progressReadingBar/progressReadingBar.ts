class ProgressBarController {
  ...
  getTargetElements = (target: string) => {
    return document.querySelectorAll(target);
  };

  addProgressBar = (root: HTMLElement) => {
    console.log("bar");
  ...
  handleScroll = (item: HTMLElement) => {
    let width =
      ((document.body.scrollTop || document.documentElement.scrollTop) /
        (document.documentElement.scrollHeight -
          document.documentElement.clientHeight)) *
      100;
    const bar2 = item.firstChild as HTMLElement;
    const bar = document.querySelector(".reading-progress-fill") as HTMLElement;
    if (bar2) bar2.style.setProperty("width", width + "%");
  };
